...
function CraftItemInfo.GetItemEffect(itemData,bNoGrowsPrefix)
  ...
  local vData
  local byId = false
  if CraftValueTip.UniqueIDList[itemData.uniqueID]~=nil then
    byId = true
    bNoGrowsPrefix = false
  end
  ...
  if itemData.cultivationType and (itemData.cultivationType>0) then
    ...
    if byId then
      --d("Get ID effect")
      if CraftValueTip.UniqueIDList[itemData.uniqueID]~=nil then
        sTemp=GetPhrase("EffectNames", CraftValueTip.UniqueIDList[itemData.uniqueID])
        return sTemp
      else
        -- dunno what's that
        return L""
    ...
    if vData[BONUS.EFFECT] and vData[BONUS.CRAFTING_FAMILY] then
      ...
      if (vData[BONUS.CRAFTING_FAMILY][1]==GameData.TradeSkills.APOTHECARY) and CraftValueTip.ApothEffectList[vData[BONUS.EFFECT][1]] then
      ...
      elseif (vData[BONUS.CRAFTING_FAMILY][1]==GameData.TradeSkills.TALISMAN) and CraftValueTip.TalEffectList[vData[BONUS.EFFECT][1]] then
